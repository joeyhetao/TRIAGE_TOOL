...
| 上传文件清理 | 上传模式：解析完成后**保留**于 `uploads/`，由启动时扫描清理 24 小时以上旧文件（v4.5 起为支持 P2 AI 日志问答跨多次请求复用文件——会话 `_store` TTL 2 小时，过期后用户已看不到结果页） |
...
| L-04 | ~~上传文件永久保存在 `uploads/` 目录，无自动清理机制~~ | **v1.2 已解决**：启动时清理 24 小时以上旧文件。注：v4.5 起为支持 P2 AI 日志问答，**取消了"解析完即删"**，恢复为完全依赖 24h 启动扫描清理 |
...
| L-08 | ~~上传文件清理仅在启动时执行一次，长期不重启时文件持续累积~~ | **v1.2 部分解决 / v4.5 复发**：v1.2 引入"解析完即删"消除累积，v4.5 因 P2 需要保留文件而取消，重新回到启动扫描清理依赖——长期不重启风险复发 |
